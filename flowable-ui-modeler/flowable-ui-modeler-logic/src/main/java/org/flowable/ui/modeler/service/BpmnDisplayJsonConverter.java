...
public class BpmnDisplayJsonConverter {
    ...
    //editor转task 测试
    public void processProcessElements2(BpmnModel pojoModel, ObjectNode displayNode, GraphicInfo diagramInfo) {
//        BpmnModel pojoModel = null;
//        BpmnJsonConverter bpmnJsonConverter=new BpmnJsonConverter();
        ObjectNode pojoModelJson = bpmnJsonConverter.convertToJson(pojoModel);
        if (pojoModelJson!=null) {
            try {
                JsonNode modelNode = objectMapper.readTree(String.valueOf(pojoModelJson));
//                pojoModel = bpmnJsonConverter.convertToBpmnModel(modelNode);
            } catch (Exception e) {
                LOGGER.error("Error transforming json to pojo ", e);
            }
        }
