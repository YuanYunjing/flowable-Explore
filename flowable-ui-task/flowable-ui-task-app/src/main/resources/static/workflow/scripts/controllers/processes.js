...
angular.module('flowableApp')
    .controller('ProcessesController', ['$rootScope', '$scope', '$translate', '$http', '$timeout', '$location', '$modal', '$routeParams', '$popover', 'appResourceRoot', 'AppDefinitionService', 'ProcessService',
        ...
        $scope.selectProcessDefinition = function (definition) {
            ...
            $scope.newProcessInstance.category = definition.category + ' - ' + new moment().format('MMMM Do YYYY');
            $scope.newProcessInstance.version = definition.version + ' - ' + new moment().format('MMMM Do YYYY');
            $scope.newProcessInstance.description = definition.description + ' - ' + new moment().format('MMMM Do YYYY');
            ...
            $scope.selectProcessDefinition1 = function (aid1,aid2) {

                ProcessService.getProcessDefinitions1(aid1,aid2).then(function(response) {
