...
app.controller('RmdProcessesController', ['$rootScope', '$scope', '$q', '$translate', '$http', '$timeout', '$location', '$modal', '$routeParams', '$popover', 'appResourceRoot', 'ProcessService',
...
//            }, 20);
        ...
        $scope.selectProcessDefinition2 = function (aid1,aid2) {
            ...
            $scope.selectProcessDefinition3 = function (aid3) {
                //因为是ng-clude 可以直接从前边页面获取对应的taskid和实例processInstanceId
                var aid1=$scope.selectedItem.properties[0].value;
                var aid2=$routeParams.processInstanceId;
                ProcessService.getProcessDefinitions3(aid1,aid2,aid3).then(function(response) {
                    $rootScope.root.processDefinitions = response.data;
                    if ($scope.root.processDefinitions && $scope.root.processDefinitions.length > 0) {
                        for (var i=0; i< $scope.root.processDefinitions.length; i++) {
                            var def = $scope.root.processDefinitions[i];
...
//            $rootScope.root.processDefinitionId1 = processDefinitionId;
            ...
            ProcessService.getProcessDefinition(processDefinitionId).then(function(response) {
               // 每一个属性都要单独赋值
                $scope.newProcessInstance=response;
