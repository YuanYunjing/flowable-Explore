...
angular.module('flowableApp')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', '$window','$q','editorManager', 'RMDServeService',
        function ($rootScope, $scope, $http, $modal, $timeout, $window, $q, editorManager, RMDServeService) {
            ...
            $scope.editorFactory.promise.then(function () {
                ...
                for (var stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) {
                    ...
                    if (currentGroupName === 'Diagram' || currentGroupName === 'BPMN.STENCILS.GROUPS.DIAGRAM' ||
                        ...
                        currentGroupName === 'BPMN.STENCILS.GROUPS.STARTEVENTS' ||
                        currentGroupName === 'BPMN.STENCILS.GROUPS.BOUNDARYEVENTS' ||
                        currentGroupName === 'BPMN.STENCILS.GROUPS.INTERMEDIATECATCHINGEVENTS' ||
                        currentGroupName === 'BPMN.STENCILS.GROUPS.INTERMEDIATETHROWINGEVENTS' ||
                        currentGroupName === 'BPMN.STENCILS.GROUPS.SWIMLANES') {
                ...
                editorManager.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                    ...
                    if (shapes && shapes.length > 0) {
                        ...
                        // 服务目录代码------------------------------------
                        // 服务目录拖拽drop后赋值时要用到$scope.selectedShape
                        // 因此将$scope.selectedShape赋值放到前面
                        $scope.safeApply(function () {
                            $scope.selectedShape = selectedShape;
                        });
                        //选中元素属性初始化：初始化种类：title、name、输入参数、任务监听器（apiHandler）
                        if($scope.item){
                            selectedItem.title = $scope.item.name;
                            var inputParamsEntityList=$scope.item.inputParamsEntityList;
                            var service_name=$scope.item.service_name;//与哈工大统一命名统一，其实是service_id

                            //拖拽过程中填写属性"名称" 根据下标确定具体属性，下标为0的属性为”名称“【以下】
                            var indexs = [0];
                            // url:$scope.item.accessPoint
                            var values = [$scope.item.name,];
                            for(var i=0; i<indexs.length; i++){
                                var property = selectedItem.properties[indexs[i]];
                                console.log(property);
                                property = Object.assign({}, property, {value: values[i]});
                                selectedItem.properties[indexs[i]] = property;
                                $scope.updatePropertyInModel(property);
                            }
                            //拖拽过程中填写属性"名称" 根据下标确定具体属性，下标为1的属性为”输入参数“【以上】

                            //拖拽过程中填写属性"任务监听器" 根据下标确定具体属性，下标为18的属性为”任务监听器“【以下】

                            // 构造选中元素”selectedItem“.taskListeners并将构造好的apiHandler增加至selectedItem.taskListners
                            // 将selectedItem.taskListeners赋值给selectedItem.properties[18].value.taskListeners,并更新至属性中

                            // Put json representing form properties on selectedItem
                            if (selectedItem.properties[18].value !== undefined && selectedItem.properties[18].value !== null && selectedItem.properties[18].value.taskListeners !== undefined && selectedItem.properties[18].value.taskListeners !== null) {
                                if (selectedItem.properties[18].value.taskListeners.constructor == String) {
                                    selectedItem.taskListeners = JSON.parse(selectedItem.properties[19].value.taskListeners);
                                }
                                else {
                                    // Note that we clone the json object rather then setting it directly,
                                    // this to cope with the fact that the user can click the cancel button and no changes should have happened
                                    selectedItem.taskListeners = angular.copy(selectedItem.properties[19].value.taskListeners);
                                }

                                for (var i = 0; i < selectedItem.taskListeners.length; i++) {
                                    var taskListener = selectedItem.taskListeners[i];
                                ...
                                selectedItem.taskListeners = [];
                            }

                            selectedItem.properties[18].value={};
                            ...
                            var newAPIListener={
                                ...
                                className:'',
                                expression:'',
                                delegateExpression:'${apiHandler}'
                            };
                            selectedItem.taskListeners.push(newAPIListener);
                            ...
                            if(selectedItem.taskListeners!=null) {
                                for (var i = 0; i < selectedItem.taskListeners.length; i++) {
                                    if (selectedItem.taskListeners[i].delegateExpression == '${apiHandler}') {
                                        if(selectedItem.taskListeners[i].fields==undefined){
                                            selectedItem.taskListeners[i].fields=[];
                                        }
                                        selectedItem.taskListeners[i].fields.push(service_name);
                                        selectedItem.taskListeners[i].fields.push(properties);

                                        if(inputParamsEntityList!=undefined && inputParamsEntityList.length!=null){//存在输入参数 先将参数名称加入properties的值中
                                            if(inputParamsEntityList.length==1){
                                                properties.stringValue=inputParamsEntityList[0].parameterName;//初始化
                                                // selectedItem.taskListeners[i].fields.push(properties);
                                                var input0={
                                                    name:inputParamsEntityList[0].parameterName,
                                                    implementation: '',
                                                    stringValue:'',
                                                    expression: '',
                                                    string:''
                                                }
                                                selectedItem.taskListeners[i].fields.push(input0);

                                            }else {
                                                for(var j=1;j<inputParamsEntityList.length;i++){//若参数>1,动态构造输入参数
                                                    properties.stringValue=","+inputParamsEntityList[j].parameterName;
                                                    // selectedItem.taskListeners[i].fields.push(properties);

                                                    var inputs="input"+j;
                                                    var input={
                                                        name:inputParamsEntityList[j].parameterName,
                                                        implementation: '',
                                                        stringValue:'',
                                                        expression: '',
                                                        string:''
                                                    }
                                                    selectedItem.taskListeners[i].fields.push(input);
                                                }//动态构造输入参数完成
                                            }//end if 参数>1
                                        }
                                        selectedItem.taskListeners[i].fields.push(output_var);
                                    }//end if apiHandler is not existed
                                }//end eachfor taskListeners
                            }//end if taskListeners!=null

                            //end 动态写好任务监听器
                            // console.log(selectedItem.taskListeners);
                            // console.log(selectedItem.properties[18].value.taskListeners)//undefined

                            //将写好的任务监听器写至属性”任务监听器“中
                            selectedItem.properties[18].value.taskListeners=selectedItem.taskListeners;//可能数据格式不匹配【已解决】

                            $scope.updatePropertyInModel(selectedItem.properties[18]);//更新
                        }
                        $scope.item = undefined; // 此处将$scope.item内容置空，否则会影响后面点击节点的属性值
                        // --------------------------------------------------
                ...
                $scope.recommend = function() {
                     //通过调用对应的taskid获取对应的值
                        var taskid1=$scope.selectedItem.properties[0].value;
                        var url = FLOWABLE.CONFIG.contextRoot + '/app/rest/rmd-process-definitions?actId=' + taskid1;
                        var request = jQuery.ajax({
                            type: 'get',
                            url: url
                        });
                        request.success(function(response){
                            var rmdList = response.data;
                            $scope.$apply(function(){
                                $scope.root.rmdProcessDefinitions = rmdList;
                                $scope.newProcessInstance = rmdList[0];
                                $scope.newProcessInstance.processDefinitionId = rmdList[0].id;
                                $scope.newProcessInstance.name = rmdList[0].name;
                                $scope.newProcessInstance.description = rmdList[0].description;
                                $scope.newProcessInstance.category = rmdList[0].category;
                                $scope.newProcessInstance.version = rmdList[0].version;
                                $scope.newProcessInstance.processDefinition= rmdList[0];
                            })
                        });
                ...
                $scope.recommend = function() {
                    // 根据服务名称获得推荐内容
                    var taskName = $scope.selectedItem.title
                    var url = FLOWABLE.CONFIG.contextRoot + '/app/rest/rmd-serves?actName=' + taskName;

                    $http({method: 'Get', url: url}).success(function(response) {  // 此处如果用jQuery.ajax会引起click无效（click后要单击空白处才有效）


                        // 推荐出的列表
                        var rmdList = response.data;
                        //调用画图代码 by Gaojian Chen
                        $scope.$broadcast('recall', rmdList,taskName);
                        if(rmdList.length!==0){
                            $scope.root.rmdServes = rmdList;
                            $scope.selectedRMDServe = {};
                            $scope.selectedRMDServe.serviceID = rmdList[0].serviceID;
                            $scope.selectedRMDServe.serviceName = rmdList[0].serviceName;
                            $scope.selectedRMDServe.accessPoint = rmdList[0].accessPoint;

                        // 根据id获得列表中第一个元素的输入输出等信息
                        $http({method: 'Get', url: FLOWABLE.CONFIG.contextRoot + '/app/rest/service-contents/services/serviceInfo' + '?id=' + rmdList[0].serviceID})
                        .success(function (responseInput) {
                            let stencilItem = $scope.getStencilItemById('UserTask');

                            // 此处不直接赋值，而是赋值到value属性中的原因是：
                            // 便于子controller使用，以防覆盖父controller（当前controller）中的该变量
                            $scope.userTask = {
                                value: Object.assign({}, stencilItem,
                                    {
                                        name: $scope.selectedRMDServe.serviceName,
                                        accessPoint: $scope.selectedRMDServe.accessPoint,
                                        service_name:rmdList[0].serviceID,
            ...
            // $scope.getServiceInfoResult=function (processInstanceId,key) {
            //     var deferred = $q.defer();

            //     $http({method: 'GET', url: FLOWABLE.URL.getServiceInfoResult(processInstanceId,key)}).
            ...
            // },
                $scope.IdentifySelectedNode=function(){
                if($scope.selectedItem.properties[18].value!=undefined){
                    $scope.selectedTaskListensers=$scope.selectedItem.properties[18].value.taskListeners;
                }else{
                    alert("未选中任何节点，请选中节点进行操作")
                }
                },
            $scope.OutputShow =function() {
                    //请求后台的方法，返回对应的值
                    var modelMetaData = editorManager.getBaseModelData();
                    var processInstanceId = modelMetaData.modelId;
                    // console.log(modelMetaData.modelId);
                    var selectedTaskListensers = $scope.selectedItem.properties[18].value.taskListeners;
                    $scope.Key = selectedTaskListensers[0].fields[0].stringValue;
                    console.log($scope.Key);
                    //获取对应的值去请求
                    $http({
                        method: 'GET',
                        url: FLOWABLE.URL.getServiceInfoResult(processInstanceId, $scope.Key)
                        ...
                        if (typeof (data[0]) == "string") {
                            ...
                            _internalCreateModal({
                                backdrop: true,
                                keyboard: true,
                                template: 'editor-app/popups/output-show-string.html?version=' + Date.now(),
                                scope: $scope
                            }, $modal, $scope);
                            ...
                            _internalCreateModal({
                                backdrop: true,
                                keyboard: true,
                                template: 'editor-app/popups/output-show-object.html',
                                scope: $scope
                            }, $modal, $scope);
                        }
                    }).error(function () {
                        console.log('存在某些错误，无法得到结果');
                    });
                }
            $scope.createInputParamsForm =function(){
                ...
                if(selectedTaskListensers){
                    for(var i=0;i<selectedTaskListensers.length;i++){
                        if(selectedTaskListensers[i].delegateExpression == '${apiHandler}'){
                            $scope.inputForm_input=selectedTaskListensers[i].fields[1].stringValue;
                            $scope.outputVarKey=selectedTaskListensers[i].fields[0].stringValue;

                            if( $scope.inputForm_input){//存在输入
                                for(var j=0;j<selectedTaskListensers[i].fields.length;j++){
                                    if(selectedTaskListensers[i].fields[j].name==selectedTaskListensers[i].fields[1].stringValue){//有输入参数
                                        $scope.inputForm.inputParamsName=selectedTaskListensers[i].fields[j].name;
                                        $scope.inputForm.inputParamsType="String";
                                        var modalInstance = _internalCreateModal({
                                            template: 'editor-app/popups/input-Params-Form.html',
                                            scope: $scope,
                                            show: true
                                        }, $modal, $scope);
                                    ...
                                    $scope.inputForm.inputParamsValue='';
                                }
                            }else{
                                // $scope.inputForm.inputParamsName = undefined;
                                // $scope.inputForm.inputParamsType="String";
                                var modalInstance = _internalCreateModal({
                                    template: 'editor-app/popups/input-Params-Form.html',
                                    scope: $scope,
            ...
            $scope.propertyClicked = function (index) {
                console.log("进来了");
                console.log($scope.selectedItem.properties[index].value);
            ...
            $scope.dropCallback = function (event, ui) {
                ...
                if ($scope.dragCanContain) {
                    ...
                    // 服务目录 id判断
                    if(item == undefined){
                        item = JSON.parse(ui.draggable[0].id);
                        $scope.item = item;
                    }
                    ...
                    for (var i = 0; i < stencilSets.length; i++) {
                        ...
                        var nodes = stencilSet.nodes();
                        for (var j = 0; j < nodes.length; j++) {
                            if (nodes[j].idWithoutNs() === item.id) {  // if (nodes[j].idWithoutNs() === ui.draggable[0].id) {
                                containedStencil = nodes[j];
                                break;
                            }
                        }
                        //console.log("containedStencil "+containedStencil);
                        if (!containedStencil) {
                            var edges = stencilSet.edges();
                            //console.log("edges "+edges+"        "+edges.length);
                            for (var j = 0; j < edges.length; j++) {
                                if (edges[j].idWithoutNs() === item.id) {  // if (edges[j].idWithoutNs() === ui.draggable[0].id) {
                    ...
                    if ($scope.quickMenu) {
                        ...
                        if (shapes && shapes.length == 1) {
                            ...
                            var option = {};
                            option.type = currentSelectedShape.getStencil().namespace() + item.id;  // option.type = currentSelectedShape.getStencil().namespace() + ui.draggable[0].id;
            ...
            $scope.dragCallback = function (event, ui) {
                ...
                if ($scope.dragModeOver != false) {
                    ...
                    if (aShapes.length == 1 && aShapes[0] instanceof ORYX.Core.Canvas) {
                        ...
                        // 服务目录 id判断
                        if(item == undefined){
                            item = JSON.parse(event.target.id);
                        }
