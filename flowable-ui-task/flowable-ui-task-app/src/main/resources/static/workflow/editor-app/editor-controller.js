    ...
    .controller('EditorController', ['$rootScope', '$scope', '$http', '$q', '$routeParams', '$timeout', '$location', '$translate', '$modal', 'editorManager',
        function ($rootScope, $scope, $http, $q, $routeParams, $timeout, $location, $translate, $modal, editorManager) {
            ...
            if (!$rootScope.editorInitialized) {
                ...
                /**
                 * Initialize the event bus: couple all Oryx events with a dispatch of the
                 * event of the event bus. This way, it gets muc0h easier to attach custom logic
                 * to any event.
                 */
                ...
                jQuery(window).resize(function () {
                    ...
                    var rmdSectionHeight = jQuery('#recommendSection').height();  // change
                    var propSection = jQuery('#propertySection');  // change
                    var sibSection = jQuery('#selected-item-body');  // change
                    var canvas = jQuery('#canvasSection');
                    var mainHeader = jQuery('#main-header');

                    if (offset == undefined || offset === null
                        || rmdSectionHeight === undefined || rmdSectionHeight === null
                    ...
                    totalAvailable = totalAvailable - 41;  // change > and
                    canvas.height(totalAvailable - 1 - rmdSectionHeight);
                    propSection.height(totalAvailable + 1);
                    sibSection.height(totalAvailable - 60);
                    var footerHeight = jQuery('#paletteSectionFooter').height();
                    var treeViewHeight = jQuery('#service-contents-wrapper').height();  // change
                    var solutionViewHeight = jQuery('#service-solution-contents-wrapper').height();  // change > and
                    jQuery('#paletteSection').height(totalAvailable - treeViewHeight - footerHeight - solutionViewHeight);  // change
            ...
            initScrollHandling();
           //注意这里边的modelid全部为实例id
            var modelId = $routeParams.processInstanceId;
