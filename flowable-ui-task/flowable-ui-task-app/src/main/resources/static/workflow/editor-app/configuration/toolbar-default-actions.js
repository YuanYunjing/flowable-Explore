...
FLOWABLE.TOOLBAR = {
    ACTIONS: {
        ...
        evolutionProcess: function(services) {

            _internalCreateModal({
                backdrop: true,
                keyboard: true,
                template: 'editor-app/popups/evolution-Process.html?version=' + Date.now(),
                scope: services.$scope
            }, services.$modal, services.$scope);
        },

        runProIns: function(services){
            ...
            services.$http({
                method: 'POST',
                url: FLOWABLE.CONFIG.contextRoot + '/app/rest/task-run-test/'+modelMetaData.modelId,
            }).success(function () {
                    console.log("运行成功");
                    services.$rootScope.addAlertPromise(services.$translate('TASK.ALERT.COMPLETED'));
...
angular.module('flowableApp').controller('SaveModelCtrl', [ '$rootScope', '$scope', '$http', '$route', '$location', 'editorManager',
...
angular.module('flowableApp').controller('ValidateModelCtrl',['$scope', '$http', 'editorManager','$timeout','$window',
...
angular.module('flowableApp').controller('EvolutionProcessInstanceCtrl', [ '$rootScope', '$scope', '$http', '$route', '$location', 'editorManager',
    function ($rootScope, $scope, $http, $route, $location, editorManager) {
        ...
        $scope.evolution = function (successCallback) {
            ...
            $http({  method: 'POST',
                ...
                url: FLOWABLE.URL.evolutionModel(modelMetaData.modelId)})
                .success(function (data, status, headers, config) {
                    editorManager.handleEvents({
                        type: ORYX.CONFIG.EVENT_SAVED
                    });
            ...
            window.location.reload();
            // jQuery('.service-solution-contents-header').load(location.href + ' .service-solution-contents-header');
