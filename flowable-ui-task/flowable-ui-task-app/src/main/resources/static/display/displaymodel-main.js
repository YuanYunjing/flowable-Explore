...
var modelDiv = jQuery('#bpmnModelMain');
...
var isDebuggerEnabled = angular.element(document.querySelector('#bpmnModelMain')).scope().model.isDebuggerEnabled;
...
function _addHoverLogic(element, type, defaultColor, elementId)
{
    var modelDiv = jQuery('#bpmnModelMain');
    var modelId = modelDiv.attr('data-model-id');
    var historyModelId = modelDiv.attr('data-history-id');
    var processDefinitionId1 = modelDiv.attr('data-process-definition-id');
    ...
    //             rmdList = response.data
    //             console.log(rmdList)
    //             var scope = angular.element('#rmd-list').scope();
    //             scope.$apply(function(){
    //                 scope.root.processDefinitions = rmdList;
    //
    //             })
    //             //
    //             var scope1 = angular.element('#rmd-list1').scope();
    //             scope1.$apply(function(){
    //                 scope1.root.processDefinitions = rmdList;
    //                 scope1.root.elementId = element.processDefinitions;
    //             })
    //         });
    //
    //     }
    ...
    topBodyRect.click(function(){
        console.log(element);
        var $scope = angular.element('#rmdController').scope();
        $scope.noRmdMessage = null;
        $scope.root.rmdProcessDefinitions = null;
        $scope.model.mode='process-create';
        if(!element.completed && (element.type=='ServiceTask'|| element.type=='UserTask' || element.type=='ScriptTask')){
            $scope.processInstanceId = modelId;
            $scope.actId = element.id;
            var url = FLOWABLE.CONFIG.contextRoot + '/app/rest/rmd-process-definitions?actId=' + element.id;
            var request = jQuery.ajax({
                type: 'get',
                url: url
            });
        ...
        if (element.current || element.brokenExecutions) {
            topBodyRect.click(function () {
                if (selectedElement != element.id) {
                    ...
                    paper.getById(element.id).attr({"stroke": "green"});
                    var scope = angular.element(document.querySelector('#bpmnModelMain')).scope();
...
function _executionClicked(activityId) {
    var executions = angular.element(document.querySelector('#bpmnModelMain')).scope().model.executions;
...
function _breakpointRestCall(actionType, activityId) {
    $.ajax({
        ...
        success: function () {
            paper.clear();
            angular.element(document.querySelector('#bpmnModelMain')).scope().getEventLog();
            _showProcessDiagram();
        }
    })
}
...
function _drawContinueExecution(x, y , executionId, activityId) {
    ...
    arrow.click(function () {
            $.ajax({
                ...
                success: function () {
                    paper.clear();
                    var processInstanceId = angular.element(document.querySelector('#bpmnModelMain')).scope().model.processInstance.id;
                    modelDiv.attr("selected-execution", processInstanceId);
                    angular.element(document.querySelector('#bpmnModelMain')).scope().model.selectedExecution = processInstanceId;
                    angular.element(document.querySelector('#bpmnModelMain')).scope().getExecutions();
                    angular.element(document.querySelector('#bpmnModelMain')).scope().model.variables = undefined;
                    angular.element(document.querySelector('#bpmnModelMain')).scope().loadVariables();
                    angular.element(document.querySelector('#bpmnModelMain')).scope().getEventLog();
                    _showProcessDiagram();
                },
                error: function () {
...
function _showProcessDiagram() {
    ...
    request.success(function (data, textStatus, jqXHR) {
        ...
        if (modelType == 'design') {
            ...
            jQuery('#bpmnModelMain').css('marginTop', offsetY);
        }

        jQuery('#bpmnModelMain').width(INITIAL_CANVAS_WIDTH);
        jQuery('#bpmnModelMain').height(INITIAL_CANVAS_HEIGHT);
        paper = Raphael(document.getElementById('bpmnModelMain'), canvasWidth, canvasHeight);
